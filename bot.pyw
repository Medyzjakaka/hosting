...
def start_attack(method, ip, port, duration, thread_count, username):
    ...
    for _ in range(thread_count):
        ...
        # ========== ДОБАВИЛ ДЛЯ STOP ==========
        attack_threads.append((t, stop_event))
        # ======================================
...
def stop_attacks(username):
    """Останавливает атаки на чужие IP, бот остается жив"""
    
    if username in user_attacks:
        for t, stop_event in user_attacks[username]:
            stop_event.set()  # Говорим потокам остановиться
        
        time.sleep(0.5)  # Ждем остановки
        
        user_attacks[username].clear()
    
    # Останавливаем все потоки в глобальном списке
    for t, stop_event in attack_threads:
        stop_event.set()
